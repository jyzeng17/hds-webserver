(function() {
	...
	function createQueryFormGroups() {
		let html = new QueryFormGroup('id').setRequired().html() +
	...
	function refreshResponseTable(url) {
		...
		if (url === 'button.send') {
			url = 'http://' + window.location.hostname +
				':8000/dataservice/v1/addmapping' + getQuery();
		}
		...
		$.get(url, function(data) {
			...
			if (jqxhr.status == 500) {
				...
				msg += '<li><b>exception:</b> ' +
					response.RemoteException.RemoteException.exception +
					'</li>';
				msg += '<li><b>message:</b> ' +
					response.RemoteException.RemoteException.message +
					'</li>';
			} else {
